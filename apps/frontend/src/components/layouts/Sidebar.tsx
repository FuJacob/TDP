...
const Sidebar = () => {
  ...
  // NEW: Collapse sidebar on smaller screens
  useEffect(() => {
    const handleResize = () => {
  ...
  return (
    <>
      ...
      <button
        className="md:hidden p-4 text-white bg-gray-800 z-50 relative"
      ...
      {/* Floating Toggle Button (high contrast) */}
      <button
      className="md:hidden fixed top-4 left-4 z-[9999] p-3 bg-blue-600 text-white rounded-full shadow-lg hover:bg-blue-700 transition-all opacity-90"
      onClick={() => setIsSidebarOpen(!isSidebarOpen)}
      aria-label="Toggle sidebar"
      >
        &#9776;
      ...
      {/* NEW: Background Overlay on Mobile */}
      {isSidebarOpen && (
        <div
      ...
      {/* Sidebar Navigation */}
      <nav
        className={`w-64 bg-gray-800 text-white p-8 space-y-4 h-screen flex flex-col fixed md:static top-0 left-0 transition-transform duration-300 z-50 ${
